...
def query_model(args: "LaunchArgs | ModelQueryArgs", prompt: list[dict]) -> str:
    ...
    # Native Gemini path: AQ-prefixed key only works against the native API,
    # not the OpenAI-compat endpoint. OpenRouter-routed Gemini stays on the
    # standard path below.
    from capx.llm.gemini_client import is_gemini_native_model
    if is_gemini_native_model(args.model):
        from capx.llm.gemini_client import query_gemini
        return query_gemini(args, prompt)  # type: ignore[return-value]
...
def query_model_streaming(
    ...
    from capx.llm.gemini_client import is_gemini_native_model
    if is_gemini_native_model(args.model):
        from capx.llm.gemini_client import query_gemini_streaming
        yield from query_gemini_streaming(args, prompt)
        return
